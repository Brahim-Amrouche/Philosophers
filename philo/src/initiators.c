...
/*   Updated: 2023/06/16 22:22:27 by bamrouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{   
    ...
    philo->params.philo_forks = malloc(sizeof(pthread_mutex_t) * philo->philo_info.philo_id);
    if (!philo->params.philo_forks)
        exit_philo("couldn't malloc philo forks\n", philo);
    philo->params.threads_id = malloc(sizeof(pthread_t) * philo->philo_info.philo_id);
    if (!philo->params.threads_id)
        exit_philo("couldn't malloc enough threads ids", philo);
    philo->params.philos_data = malloc(sizeof(t_philo_instance) * philo->philo_info.philo_id);
